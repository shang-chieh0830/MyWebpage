...
在線性回歸分析中，杠杆統計量用於評估數據點對回歸線的影響程度，其值取決於該數據點的自變量值與其他數據點的自變量值的差距大小。杠杆統計量越大，則該數據點對回歸線的影響越大。

杠杆統計量可以用數學公式計算，通常使用的是 Cook's distance 或 DFFITS 這兩種方法。Cook's distance 是評估單個數據點對回歸分析影響的指標，而 DFFITS 則是評估當刪除某個數據點時對回歸分析的影響程度。
...
update() 函數是一個用於修改 R 對象的函數。它通常用於更新模型、替換數據框中的變數、修改數據框中的觀察值等。

```{r chunk18}
# use update() to take out one variable (age) from out original model
...
in a regression fit.
 ...
 `poly()` 函數是在 R 語言中用於生成多項式特徵的函數。這些多項式特徵可以用於建立線性模型或非線性模型，並且通常是在進行特徵工程時使用的一種技術。

默認情況下，`poly()` 函數會對特徵變量進行正交化（orthogonalization），這意味著輸出的特徵不僅僅是變量的一系列次方。正交化是一種線性代數技術，可以用來減少多重共線性（multicollinearity）的影響，從而提高模型的穩定性和可靠性。

如果想要獲取原始多項式而不是正交化後的多項式，則需要在調用` poly()` 函數時將參數 raw 設置為 TRUE。這樣做可以生成原始多項式，這些多項式可以與直接計算得到的多項式使用相同的線性模型進行分析和比較。需要注意的是，正交化和未正交化的多項式之間的迴歸係數、標準誤和 p 值都會有所不同，因此在進行模型訓練和評估時需要根據具體情況選擇合適的多項式特徵形式。
