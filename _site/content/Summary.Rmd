...
|$\text{Negative Binomial}$<br><br>$X\sim NB(r,p),$ <br><br>$0<p<1, r=1,2,...$|${r+x-1\choose x}p^rq^{x},$ <br><br>$x=0,1,2,...$, <br><br>$\text{X is the number of failures}$|$rq/p$|$rq/p^2$|$(\frac{pe^t}{1-qe^t})^r$, , <br><br>$t<-\ln p$|
...
| $\text{Exponential}$<br><br>$X\sim Exp(\theta),$ <br><br>$\theta>0$  | $\frac{1}{\theta}e^{-x/\theta},$ <br><br>$x>0$                              | $\theta$             | $\theta^2$           | $\frac{1}{1-\theta t}$ |
...
4.  $Geo(p)\leftrightarrow NB(n,p)$
    ...
    A negative binomial distribution with $r=1$ is a geometric distribution, i.e., $Geo(p)=NB(1,p)$. Also, the sum of $r$ independent $Geo(p)$ random variables is a $NB(r,p)$ random variables, i.e., if $X_i\sim i.i.d.Geo(p)\text{, then }\sum_{i=1}^{r} X_i\sim NB(r,p)$.
...
8. $Poi(\lambda)\leftrightarrow Gamma(n,\lambda)$
    ...
    If $X\sim Gamma(n, \beta)$, where $n$ is an integer, then for any x, we have $$P(X\le x)=P(Y\ge n),$$ where $Y\sim Poi(x/\beta)$
