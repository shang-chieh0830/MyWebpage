 ...
 * `outer(X,Y,FUN)` 函數會返回一個矩陣，該矩陣的大小為 `length(X) x length(Y)`，矩陣中的每一個元素都是由 FUN 函數對 X 和 Y 中的對應元素進行運算得到的。
...
`identify()` 函數可以在 R 語言的圖形設備（如圖形視窗）中顯示一個散佈圖或其他類型的圖形。當用戶單擊圖形中的一個點時，`identify()` 函數會返回點的位置。當用戶單擊圖形中的一個點時，該點的位置將被返回。用戶可以單擊多個點，直到按下 `ESC` 鍵為止，然後 `identify()` 函數會返回一個向量，其中包含所選點的位置。

```{r chunk36}
# type this chunk in the console or R-script
plot(horsepower, mpg)
